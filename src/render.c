...
void wireworld(SDL_Renderer *renderer, state_t *state)
{
    ...
    for (int i = 0; i < MOVES_PER_FRAME; i++) {
        ...
        for (int x = 0; x < N; x++)
            for (int y = 0; y < N; y++) {
                int n_neigh =
                    (state->board[mod((x - 1), N)][mod((y - 1), N)] == BLUE) +
                    (state->board[mod((x    ), N)][mod((y - 1), N)] == BLUE) +
                    (state->board[mod((x + 1), N)][mod((y - 1), N)] == BLUE) +
                    (state->board[mod((x - 1), N)][mod((y    ), N)] == BLUE) +
                    (state->board[mod((x + 1), N)][mod((y    ), N)] == BLUE) +
                    (state->board[mod((x - 1), N)][mod((y + 1), N)] == BLUE) +
                    (state->board[mod((x    ), N)][mod((y + 1), N)] == BLUE) +
                    (state->board[mod((x + 1), N)][mod((y + 1), N)] == BLUE);
                ...
                if (state->board[x][y] == BLACK)
                    new_board[x][y] = BLACK;
                else if (state->board[x][y] == BLUE)
                    new_board[x][y] = RED;
                else if (state->board[x][y] == RED)
                    new_board[x][y] = WHITE;
                else if (state->board[x][y] == WHITE && (n_neigh == 1 || n_neigh == 2))
                    new_board[x][y] = BLUE;
                else
                    new_board[x][y] = WHITE;
            }
